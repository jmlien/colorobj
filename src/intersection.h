...
/*---------------------------------------------------------------------
LineLineInt: Finds the point of intersection p between two lines defined by ab and cd.
   Returns p and a char with the following meaning:
   'e': The lines collinearly overlap, in this case, p is 'a'
   '1': The lines intersect properly (i.e., overlap but 'e' does not hold).
   '0': The segments do not intersect (paralel), in this p is undefined
---------------------------------------------------------------------*/
template<class real>
char LineLineInt( const real a[2], const real b[2],
                  const real c[2], const real d[2],
                  real p[2] )
{
    real  s;
